...
async function requireRatio(file, expected, label) { if (!file) return; const [width,height] = await dimensions(file); if (Math.abs(width / height - expected) > .035) throw new Error(`${label} must use a ${expected > 1 ? '16:9' : '5:7'} aspect ratio.`); }
...
async function render(root, editing = null) {
  ...
  host.innerHTML = `<section class="post-editor-shell">
    <form data-post-form>
      ...
      <div class="post-art-grid"><label class="art-upload"><span>Full Thumb <b>16:9</b></span><small>Hero, lead card, and full-width artwork</small><input type="file" name="full_thumb" accept="image/*" ${editing ? '' : 'required'}><img data-full-preview src="${esc(editing?.full_thumb_url || '/assets/images/post-fallback-full.webp')}" alt=""></label><label class="art-upload poster"><span>Poster <b>5:7</b></span><small>Header drawer and compact cards</small><input type="file" name="poster" accept="image/*" ${editing ? '' : 'required'}><img data-poster-preview src="${esc(editing?.poster_url || '/assets/images/post-fallback-poster.webp')}" alt=""></label></div>
...
function bind(root, posts) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      await requireRatio(form.elements.poster.files[0], 5 / 7, 'Poster');
